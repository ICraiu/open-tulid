...
Use the injected implementation-spec template as the structure and the injected diagram-requirements reference to decide which diagrams are required. Inspect repository files in the workspace when they exist, and use the injected repository facts and unfinished-task inventory: the specification must account for the code that is already written and the existing agreed work so the project can iterate instead of restarting from a blank design. Inspect actual manifests and integration seams, distinguish implemented behavior from remaining work, and preserve interfaces already promised by prerequisite tasks.
